...
// add a mapping to the user kpagetable.
...
//new a kernel pagetable for each process
pagetable_t
ukvminit()
{
...
kvmpa(uint64 va)
{
  ...
  pte = walk(myproc()->kpagetable, va, 0);//kernel pagetable
  if(pte == 0)
    panic("kvmpa");
  if((*pte & PTE_V) == 0)
    panic("kvmpa");
...
u2kvmcopy(pagetable_t upagetable, pagetable_t kpagetable, uint64 oldsz,uint64 newsz)
{
    ...
    uint64 pa, i;
    uint flags;
    uint64 begin_page = PGROUNDUP(oldsz);    // 向上取整
    for(i = begin_page; i < newsz; i += PGSIZE){
...
err:
    uvmunmap(kpagetable, oldsz, (i- oldsz) / PGSIZE, 0);
    return -1;
...
vmprint(pagetable_t pagetable,uint64 depth)
{
   ...
   if(depth==0)printf("page table %p\n",pagetable);
      for(int i = 0; i < 512; i++){
    	pte_t pte = pagetable[i];
    	if((pte & PTE_V)){//检查那些存在但没有设置任何访问权限的无效页面
      	 // this PTE points to a lower-level page table.
     	 //.. ..0: pte 0x0000000021fda401 pa 0x0000000087f69000
     		if(depth==0)printf("..%d: pte %p pa %p \n",i,pte,PTE2PA(pte));
     		else if(depth==1)printf(".. ..%d:pte %p pa %p \n",i,pte,PTE2PA(pte));
     		else if(depth==2)printf(".. .. ..%d:pte %p pa %p \n",i,pte,PTE2PA(pte));
     		uint64 child = PTE2PA(pte);
